...
main(int argc, char *argv[]) {
    ...
    if (pid == 0) {
        ...
        for (i = 0; i < 3*n; i++) {
            ...
            printf(1, "pid:%d type:%d wait_time:%d sleep_time:%d run_time:%d\n", pid, pid % 3, retime, setime, rutime);
        ...
        printf(1, "CPU bound :  ready time:%d , sleep time:%d, Turnaround Time:%d\n", total_retime_type1 / n, total_setime_type1 / n, total_setime_type1 + total_retime_type1 + total_rutime_type1);
        printf(1, "SCPU bound:  ready time:%d , sleep time:%d, Turnaround Time:%d\n", total_retime_type2 / n, total_setime_type2 / n, total_setime_type2 + total_retime_type2 + total_rutime_type2);
        printf(1, "IO bound  :  ready time:%d , sleep time:%d, Turnaround Time:%d\n", total_retime_type3 / n, total_setime_type3 / n, total_setime_type3 + total_retime_type3 + total_rutime_type3);
    }
    exit();
}
